...
use archstore_core::backend::{PackageBackend, PacmanBackend};
...
async fn main() -> std::process::ExitCode {
    ...
    if args.len() < 3 {
        eprintln!("用法：make-plan <输出目录> <install|remove> <包名> [仓库]");
        return std::process::ExitCode::from(2);
    }
    let out_dir = PathBuf::from(&args[0]);
    let action = args[1].clone();
    let name = args[2].clone();
    let repo = args.get(3).cloned().unwrap_or_else(|| "extra".to_string());

    let backend: Arc<dyn PackageBackend> = match PacmanBackend::spawn().await {
        Ok(b) => Arc::new(b),
        Err(e) => {
    ...
    let id = PackageId::official(repo, name.clone());
    ...
    let result = match action.as_str() {
        ...
        other => {
            eprintln!("未知动作：{other}（应为 install 或 remove）");
            return std::process::ExitCode::from(2);
        }
    };
